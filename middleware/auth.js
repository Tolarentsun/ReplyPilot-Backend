...
router.post('/register', async (req, res) => {
  try {
    ...
    db.prepare(`
      INSERT INTO users (id, name, email, password_hash, business_name, business_type, plan)
      VALUES (?, ?, ?, ?, ?, ?, 'free')
    `).run(id, name, email.toLowerCase(), passwordHash, business_name || null, business_type || null);

    // Seed demo review data for new users
...
router.put('/profile', authenticate, (req, res) => {
  try {
    ...
    db.prepare(`
      UPDATE users SET name = ?, business_name = ?, business_type = ?, updated_at = datetime('now')
      WHERE id = ?
