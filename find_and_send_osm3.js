/**
 * OSM v3: concurrent workers, 3s timeout, homepage-only scan, skip slow sites fast
 */
...
const BATCH_OUT = BASE + 'sent-batch6-2026-05-03.txt';
...
const JUNK_DOMAINS = new Set([
  ...
  'ingest.us.sentry.io','ingest.sentry.io','gist-apps.com'
]);
...
// Remaining cities (Seattle + Portland already done)
const CITIES = [
...
// Fast fetch with 3-second timeout, no retry
...
async function getEmailFast(siteUrl) {
  ...
  // Only check homepage and /contact — 2 pages max
...
<table style="width:100%;border-collapse:collapse;margin:16px 0">
  <tr>
    <td style="padding:10px 12px;background:#f9f7f4;border-radius:6px;margin-bottom:8px;display:block">
...
async function processCity(city, globalState) {
  ...
  await delay(1200); // respect Overpass
  ...
  await mapConcurrent(businesses, async (biz) => {
    ...
    // Lock check (single threaded in JS so no real race condition)
  ...
  }, 5); // 5 concurrent workers per city
}

async function main() {
  console.log('OSM v3 — concurrent workers (5), 3s timeout');
